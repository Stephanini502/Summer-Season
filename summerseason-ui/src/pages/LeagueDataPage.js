...
function LeagueDataPage() {
  ...
  const rankClass = (i) =>
    i === 0 ? "pg-rank pg-rank-1" : i === 1 ? "pg-rank pg-rank-2" : i === 2 ? "pg-rank pg-rank-3" : "pg-rank";
  ...
  return (
    <>
      <style>{sharedStyles}
        ...
        .media-video { width: 100%; border-radius: 10px; max-height: 220px; display: block; }
        ...
        .challenge-media-wrap { padding: 8px 16px 16px; border-top: 1px dashed var(--border); background: #fafbff; }
      ...
      <div className="pg-root">
        <div className="pg-content">
          ...
          <div className="pg-stats">
            ...
            <div className="pg-stat-card">
              <div><p className="pg-stat-label">Punti ottenibili</p><p className="pg-stat-value">{totalPts}</p></div>
            ...
            <div className="media-section">
              <div>
                ...
                <button className="media-upload-btn" onClick={() => triggerUpload("league", league.id, "image")}>
                  📷 {leagueImages.length > 0 ? "Aggiungi altra immagine" : "Carica immagine"}
                </button>
              </div>
              ...
              <div>
                ...
                <button className="media-upload-btn" onClick={() => triggerUpload("league", league.id, "video")}>
                  🎬 {leagueVideos.length > 0 ? "Aggiungi altro video" : "Carica video"}
                </button>
              </div>
            </div>
          </div>
                        ...
                        <div className="challenge-media-wrap">
                          {cImages.length > 0 && (
                            <div className="media-grid" style={{ marginBottom: 8 }}>
                              {cImages.map((m, idx) => (
                                <div key={m.id} className="media-item">
                                  <img src={m.url} alt={c.name} className="media-thumb" style={{ maxHeight: 120 }} onClick={() => openLightbox(cImages, idx)} />
                                  <div className="media-item-overlay">
                          ...
                          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                            <button className="media-upload-btn" onClick={() => triggerUpload("challenge", c.id, "image")}>
                              📷 {cImages.length > 0 ? "Aggiungi immagine" : "Carica immagine"}
                            </button>
                            <button className="media-upload-btn" onClick={() => triggerUpload("challenge", c.id, "video")}>
                              🎬 {cVideos.length > 0 ? "Aggiungi video" : "Carica video"}
                            </button>
                          </div>
                        </div>
                      </li>
